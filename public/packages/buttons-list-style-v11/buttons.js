(() => {
  'use strict';
  const states = Object.freeze([{"value":"bullets","label":"Use numbers","aria":"Change list to numbers","status":"Bulleted list."},{"value":"numbers","label":"Use checks","aria":"Change list to checks","status":"Numbered list."},{"value":"checks","label":"Use bullets","aria":"Change list to bullets","status":"Checklist."}].map(Object.freeze));
  function mount(root, options = {}) {
    ...
    function setValue(value, emit = false) {
      ...
      render();
      if (emit) {
        const detail = { kind: root.dataset.kind, listStyle: states[index].value };
        options.onChange?.({ listStyle: states[index].value });
    ...
    reset();
    return { cycle: activate, setListStyle: value => setValue(value), reset, destroy, get state() { return { listStyle: states[index].value, index }; } };
  }
