(() => {
  'use strict';
  const states = Object.freeze([{"value":"regular","label":"Italic","aria":"Turn on italic text","status":"Upright text."},{"value":"italic","label":"Regular","aria":"Turn off italic text","status":"Italic text."}].map(Object.freeze));
  function mount(root, options = {}) {
    ...
    function setValue(value, emit = false) {
      ...
      render();
      if (emit) {
        const detail = { kind: root.dataset.kind, style: states[index].value };
        options.onChange?.({ style: states[index].value });
    ...
    reset();
    return { toggle: activate, setStyle: value => setValue(value), reset, destroy, get state() { return { style: states[index].value, index }; } };
  }
