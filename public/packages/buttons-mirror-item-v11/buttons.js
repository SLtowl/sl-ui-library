(() => {
  'use strict';
  const states = Object.freeze([{"value":"normal","label":"Mirror","aria":"Mirror item horizontally","status":"Item is not mirrored."},{"value":"mirrored","label":"Restore","aria":"Restore item orientation","status":"Item is mirrored."}].map(Object.freeze));
  function mount(root, options = {}) {
    ...
    function setValue(value, emit = false) {
      ...
      render();
      if (emit) {
        const detail = { kind: root.dataset.kind, mirrored: states[index].value };
        options.onChange?.({ mirrored: states[index].value });
    ...
    reset();
    return { toggle: activate, setMirrored: value => setValue(value), reset, destroy, get state() { return { mirrored: states[index].value, index }; } };
  }
