(() => {
  'use strict';
  const states = Object.freeze([{"value":"ready","label":"Redo action","aria":"Redo the local preview action","status":"Redo is ready."},{"value":"redone","label":"Redo again","aria":"Repeat the local preview action again","status":"Local preview completed one redo."}].map(Object.freeze));
  function mount(root, options = {}) {
    ...
    function setValue(value, emit = false) {
      ...
      render();
      if (emit) {
        const detail = { kind: root.dataset.kind, redoState: states[index].value };
        options.onChange?.({ redoState: states[index].value });
    ...
    reset();
    return { toggle, setRedoState: value => setValue(value), reset, destroy, get state() { return { redoState: states[index].value, index }; } };
  }
