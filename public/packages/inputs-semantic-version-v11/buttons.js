(() => {
  ...
  function mount(root, options = {}) {
    ...
    const fields=[q('.sl-major'),q('.sl-minor'),q('.sl-patch')],out=q('.sl-version'),next=q('.sl-next'); const incoming=Array.isArray(options.value)?options.value.map(String):fields.map(x=>x.value); const initial=incoming.length===3?incoming:fields.map(x=>x.value);let state={};
      function render(emitChange=false){const parts=fields.map(x=>x.value),valid=parts.every(x=>/^(0|[1-9]\d{0,2})$/.test(x));state={value:valid?parts.map(Number):null,valid,draft:parts.slice()};out.textContent=valid?'v'+parts.join('.'):'Invalid version';next.disabled=disabled||!valid||Number(parts[2])>=999;setValidity(valid,valid?'Semantic version is complete.':'Each segment must be an integer from 0 to 999.',fields);if(emitChange)emit();}
      fields.forEach((field,i)=>{field.value=initial[i]??'0';field.setAttribute('aria-label',['Major version','Minor version','Patch version'][i]);on(field,'input',()=>render(true));});on(next,'click',()=>{fields[2].value=String(Number(fields[2].value)+1);render(true);fields[2].focus();});render();
      return finish(()=>state,()=>{fields.forEach((x,i)=>x.value=initial[i]??'0');render();});
  }
