(() => {
  'use strict';
  const states = Object.freeze([{"value":"regular","label":"Bold","aria":"Turn on bold text","status":"Regular text."},{"value":"bold","label":"Regular","aria":"Turn off bold text","status":"Bold text."}].map(Object.freeze));
  function mount(root, options = {}) {
    ...
    function setValue(value, emit = false) {
      ...
      render();
      if (emit) {
        const detail = { kind: root.dataset.kind, weight: states[index].value };
        options.onChange?.({ weight: states[index].value });
    ...
    reset();
    return { toggle: activate, setWeight: value => setValue(value), reset, destroy, get state() { return { weight: states[index].value, index }; } };
  }
