(() => {
  ...
  function mount(root, options = {}) {
    ...
    const input=q('.sl-main'), count=q('.sl-count'); const initial=String(options.value??input.value); let current=initial;
      function render(emitChange=false){ current=input.value; const valid=/^[A-Za-z0-9._]{3,24}$/.test(current)&&!/[._]{2}/.test(current)&&!/^[._]|[._]$/.test(current); count.textContent=current.length+' / 24'; setValidity(valid, valid?'Available format.':'Use 3–24 characters without repeated or edge punctuation.', [input]); if(emitChange) emit(); }
      input.value=initial; on(input,'input',()=>render(true)); render();
      return finish(()=>({value:current,valid:root.dataset.invalid!=='true'}),()=>{input.value=initial; current=initial; render();});
  }
