...
export function assertCatalogContract({complete=false}={}) {
  ...
  for(const category of categories){
    ...
    const batchSize=category.id==='sliders'?14:category.id==='buttons'?30:10;
  ...
  if(complete)assert.equal(additions.length,124,'All approved batches are required for signoff.');
}
