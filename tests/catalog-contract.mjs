...
export function assertCatalogContract({complete=false}={}) {
  ...
  for(const category of categories){
    ...
    const batchSize=category.id==='sliders'?14:category.id==='buttons'?53:category.id==='inputs'||category.id==='selection'?20:10;
  ...
  if(complete)assert.equal(additions.length,167,'All approved batches are required for signoff.');
}
